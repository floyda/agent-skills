...
Create `tasks.md` with tasks organized by phase. Each task must have a unique ID (T001, T002, etc.) in checkbox format (`- [ ] T001: Task title`), and be atomic, specific, and completable in one focused session. Link to plan phases and requirements.
...
3. **Mark progress**: Update tasks with `[x]` as work completes
